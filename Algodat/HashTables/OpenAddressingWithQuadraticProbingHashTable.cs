...
{
    public class OpenAddressingWithQuadraticProbingHashTable<TKey, TValue> : IHashTable<TKey, TValue>
        where TValue : class
    {
        private class Node
        {
        ...
        private const int C1 = 123;
        private const int C2 = 456;


        public OpenAddressingWithQuadraticProbingHashTable()
        {
            _array = new Node[4];
        }

        private int Hash(TKey key, int i) => (key.GetHashCode() + C1 * i + C2 * i * i) % _array.Length;
